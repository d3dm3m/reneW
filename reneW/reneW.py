...
class ReneW:
    ...
    def _run_hotspot_analysis(self, analysis_configs, threshold, distance):
        ...
        # 1. Filter high-risk features from all selected layers
        for config in analysis_configs:
            layer = config['layer']
        ...
        all_hotspot_polygons = []

        # 2. Single-type hotspot analysis (finding clusters within each type)
        for layer_type, geoms in high_risk_features.items():
            if len(geoms) > 1:
                # Buffer each individual geometry
                individual_buffers = [geom.buffer(distance, 5) for geom in geoms]
                # Combine them into a single multi-part geometry
                combined_buffers = QgsGeometry.collectGeometry(individual_buffers)
                # Dissolve the buffers to merge overlapping areas into clusters
                dissolved_clusters = combined_buffers.unaryUnion()
                if not dissolved_clusters.isEmpty():
                    all_hotspot_polygons.append(dissolved_clusters)

        # 3. Cross-type hotspot analysis (finding where different types are close)
        active_types = [t for t, geoms in high_risk_features.items() if geoms]
        if len(active_types) >= 2:
            # Create a single dissolved buffer for each type
            dissolved_buffers_by_type = {}
            for layer_type, geoms in high_risk_features.items():
                if geoms:
                    combined_geom = QgsGeometry.collectGeometry(geoms)
                    buffer_geom = combined_geom.buffer(distance, 5)
                    dissolved_buffers_by_type[layer_type] = buffer_geom

            # Find intersections between the buffers of different types
            type_pairs = [
                ('Vatten', 'Spillvatten'),
                ('Vatten', 'Dagvatten'),
            ...
            for type1, type2 in type_pairs:
                if type1 in dissolved_buffers_by_type and type2 in dissolved_buffers_by_type:
                    geom1 = dissolved_buffers_by_type[type1]
                    geom2 = dissolved_buffers_by_type[type2]
                    intersection = geom1.intersection(geom2)
                    if not intersection.isEmpty():
                        all_hotspot_polygons.append(intersection)

        # 4. Finalize and combine all found hotspots
        if not all_hotspot_polygons:
            self.iface.messageBar().pushMessage("Info", "Inga hotspots hittades.", level=0, duration=3)
            return None

        # Combine all found hotspot polygons (from single and cross-type analysis)
        final_hotspots_geom = QgsGeometry.collectGeometry(all_hotspot_polygons).unaryUnion()

        hotspot_count = 0
        if final_hotspots_geom.isMultipart():
            hotspot_count = len(final_hotspots_geom.asMultiPolygon())
        elif not final_hotspots_geom.isEmpty():
            hotspot_count = 1

        self.iface.messageBar().pushMessage("Success", f"{hotspot_count} hotspot-områden identifierade.", level=0, duration=4)
