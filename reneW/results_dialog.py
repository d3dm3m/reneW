...
FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "results_dialog.ui")
)


class ResultsDialog(QDialog, FORM_CLASS):
    """Dialog for showing high-risk results."""
    ...
    def populate_table(self, results):
        ...
        for row_idx, result in enumerate(results):
            ...
            self.resultsTable.setItem(
                row_idx, 0, QTableWidgetItem(result.get("layer_name", ""))
            )
            self.resultsTable.setItem(
                row_idx, 1, QTableWidgetItem(result.get("pipe_type", ""))
            )
            self.resultsTable.setItem(
                row_idx, 2, QTableWidgetItem(result.get("material", ""))
            )
            self.resultsTable.setItem(
                row_idx, 3, NumericStringTableWidgetItem(str(result.get("age", "")))
            )
            self.resultsTable.setItem(
                row_idx,
                4,
                NumericStringTableWidgetItem(f"{result.get('renewal_need', 0.0):.2f}"),
            )
            self.resultsTable.setItem(
                row_idx,
                5,
                NumericStringTableWidgetItem(
                    f"{result.get('optimism_factor', 1.0):.2f}"
                ),
            )
            self.resultsTable.setItem(
                row_idx, 6, QTableWidgetItem(str(result.get("feature_id", "")))
            )
            self.resultsTable.setItem(
                row_idx, 7, QTableWidgetItem(str(result.get("layer_id", "")))
            )
