...
class ParameterEditorDialog(QDialog, FORM_CLASS):
    ...
    def _populate_table(self):
        ...
        param_set = next((s for s in self.data.get('parameter_sets', []) if s['name'] == selected_type), None)
        if not param_set:
            return
        ...
        for row_idx, material in enumerate(materials):
            self.mMaterialsTable.setItem(row_idx, 0, QTableWidgetItem(material.get('key', '')))
            self.mMaterialsTable.setItem(row_idx, 1, QTableWidgetItem(",".join(material.get('keywords', []))))
            self.mMaterialsTable.setItem(row_idx, 2, QTableWidgetItem(str(material.get('year_min', ''))))
            self.mMaterialsTable.setItem(row_idx, 3, QTableWidgetItem(str(material.get('year_max', ''))))
            self.mMaterialsTable.setItem(row_idx, 4, QTableWidgetItem(str(material.get('params', {}).get('a', ''))))
            self.mMaterialsTable.setItem(row_idx, 5, QTableWidgetItem(str(material.get('params', {}).get('b', ''))))
            self.mMaterialsTable.setItem(row_idx, 6, QTableWidgetItem(str(material.get('params', {}).get('c', ''))))
    ...
    def accept(self):
        ...
        for row in range(self.mMaterialsTable.rowCount()):
            ...
            try:
                ...
                material['keywords'] = [k.strip() for k in self.mMaterialsTable.item(row, 1).text().split(',')]

                year_min = self.mMaterialsTable.item(row, 2).text()
                if year_min: material['year_min'] = int(year_min)

                year_max = self.mMaterialsTable.item(row, 3).text()
                if year_max: material['year_max'] = int(year_max)
